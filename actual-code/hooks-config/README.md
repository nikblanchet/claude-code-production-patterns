...
See `settings.local.json` for a production example refined through use in large polyglot codebases.
...
The included `settings.local.json` shows a production configuration refined through use in large polyglot codebases (Python + TypeScript + JavaScript) with:
